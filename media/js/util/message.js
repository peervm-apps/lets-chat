...
(function(exports) {
    ...
    var markdownFormatter = window.markdownit('zero', {
        html: false
    }).enable([ 'backticks', 'emphasis', 'link', 'linkify' ]);

    var textFormatter = window.markdownit('zero', {
        html: false,
        breaks: false,
        linkify: true
    });
    ...
    function textFormat(text) {
        ...
        if (paste && formatted.length) {
            return '<pre>' +
                formatted.substring(3, formatted.length - 7) + '</pre>';
        }
    ...
    exports.format = function(text, data) {
        ...
        var pipeline = [
            pasted ? textFormat : markdownFormat,
            mentions,
            roomLinks,
            emotes,
            replacements
